...
const static int g_maxSize = 160;
const static int g_JPGQuality = 40;
...
//     info.err = jpeg_std_error(&err);
	...
	if (ret_Read_Head != JPEG_HEADER_OK) {
		g_logCall("jpeg_read_header failed\n");
		fclose(file);
		jpeg_destroy_decompress(&info);
		return NULL;
	}
	...
	if (!bStart) {
		g_logCall("jpeg_start_decompress failed\n");
		fclose(file);
		jpeg_destroy_decompress(&info);
		return NULL;
	}
...
bool generate_image_thumbnail(const char* inputFile, const char* outputFile, const logCall& call)
{
	...
	//读取jpeg图片像素数组
	int wSrc = 0, hSrc = 0;
	unsigned char* buff = ReadJpeg(inputFile, wSrc, hSrc);
	if (buff == NULL) {
		g_logCall("ReadJpeg Failed\n");
		g_logCall(inputFile);
		g_logCall(outputFile);
		return false;
	}

	int tb_w = wSrc, tb_h = hSrc;
	if (wSrc > g_maxSize || hSrc > g_maxSize)
	{
		int scaleJpg = wSrc > hSrc ? (wSrc / g_maxSize) : (hSrc / g_maxSize);
		tb_w = wSrc / scaleJpg;
		tb_h = hSrc / scaleJpg;
	...
	unsigned char * img_buf = do_Stretch_Linear(tb_w, tb_h, 24, buff, wSrc, hSrc);
	free(buff);

	//将缩放后的像素数组保存到jpeg文件
	bool bRetWrite = write_JPEG_file(outputFile, img_buf, g_JPGQuality, tb_w, tb_h);
	delete[] img_buf;
	...
	if (bRetWrite) {
		...
		g_logCall("GenerateImageThumbnail: write failed\n");
		return true;
	}
}